...
class OrderB2B extends React.Component {
  ...
  getListbyPaging(currPage, currLimit) {
    ...
    const urlA = `${myUrl.url_b2b}type=tomorrowMAuth&gudangID=${gudangID}`;
  ...
  componentDidMount() {
    this.props.setTitle(
      'Pesanan B2B Yang Harus Selesai dan Packing Besok Pagi (P3)',
      '#FC8B03',
    );
    this.getListbyPaging(this.state.currentPage, this.state.todosPerPage);
      ...
      currentTodos.map((todo, i) => {
        return (
          <tr key={i} style={{ width: '200%' }}>
            ...
            <td style={{ fontSize: '25px' }}>{todo.NOPL}</td>
