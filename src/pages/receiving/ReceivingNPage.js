...
import { MdSearch, MdAutorenew, MdInfo } from 'react-icons/md';
...
class ReceivingNPage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getDetailPO(group, dataPO, gudangID) {
    ...
    // console.log('FLAG N', url);
    // console.log('GROUP: ', group, typeof group);

    var payload = {
      ...dataPO,
    };
    // console.log('TEST PAYLOAD DATA PO', payload);
    ...
    fetch(url, option)
      ...
      .then(data => {
        // console.log('TESTING DATA PO AGAIN', data);
        var allData = data;
        ...
        if (error.status === false) {
          ...
          if (metadata.Status === 'TRUE') {
            ...
            // console.log("ISI HEADERPO", header);
  ...
  getDetailPOAgain(group, data, gudangID) {
    ...
    fetch(url, option)
      ...
      .then(data => {
        // console.log('GET DATA PO AGAIN LIST', data);
  ...
  getKaryawanData(nip) {
    ...
    // console.log('URL KARYAWAN', url);
    var payload = {
      nip: [nip],
    };
    // console.log('PAYLOAD KARYAWAN', payload);
    ...
    fetch(url, option)
      ...
      .then(data => {
        var allData = data;
        // console.log('ALL DATA KARYAWAN', allData);
        //if found
        if (allData.error.stauts === true) {
          ...
          if (allData.data === null) {
            ...
            if (allData.data.length > 0) {
              this.setState(
                { namaKaryawan: allData.data[0].nama, loadingKaryawan: true },
                this.showNotification(
                  'Data ' + allData.data[0].nama + ' ditemukan',
                  'info',
                ),
              );
  ...
  confirmPO = () => {
    ...
    // console.log('TEST CARI PAYLOAD', payload);
  ...
  deleteItem = todo => evt => {
    ...
    // newData.batch[index]['rcvd_nobatch'] = newData['rcvd_nobatch'];
    // var tempArray = currProd.batch;
    // tempArray.filter(
    //   deleteItem => deleteItem.rcvd_nobatch === todo.rcvd_nobatch,
    // );
    // this.setState({
    //   currProd: tempArray
    // }, console.log("ISI SETELAH TEMP: ", this.state.currProd))
  ...
  saveBatchUpdate = (newData, index) => evt => {
    ...
    var header = data1.header;
    // console.log("HEADER RECV", header)
    ...
    newData.batch[index]['rcvd_quantitysellunitpo'] =
      newData['rcvd_quantitysellunitpo'];
    newData.batch[index]['rcvd_quantitysellunitbonuspo'] =
      newData['rcvd_quantitysellunitbonuspo'];
    ...
    var qtyPO = newData['rcvd_quantitysellunitpo'];
    var qtyBonusPo = newData['rcvd_quantitysellunitbonuspo'];
    var totalQty = 0;
  ...
  updateObjectValue(value, field, Data) {
    ...
    this.setState(
      { currentData },
      // () =>
      // console.log('CURRENT DATA INPUT', this.state.headerPO),
    );
  }
      ...
      if (found === undefined) {
        ...
        if (realCurrProcod !== tempProcod && found['batch'].length > 1) {
          ...
          if (found.rcvd_passIc !== '') {
            ...
            if (realCurrProcod !== tempProcod) {
              ...
              if (found['batch'][0]['rcvd_quantityrecv'] <= quantity + 1) {
                ...
                // console.log('SET CURR PROD', setCurrProd);
                this.setState(
                  {
                    realCurrProcod: tempProcod,
              ...
              console.log('SET CURR PROD 2', setCurrProd);
          ...
          this.setState(
            { show: true },
            () =>
              document.getElementById('qtyPOBonus').style.display === 'block',
          );
        }
      }
    } else if (
  ...
  getEnterForKaryawan(param) {
    this.setState({ loadingKaryawan: false }, () =>
      this.getKaryawanData(param),
    );
  }
  ...
  render() {
    ...
    if (currentTodos.length > 0) {
      ...
      // console.log('DISPLAY', x && x.style.display);
    }

    if (this.isEmpty(currProd) || currProd.rcvd_nobatch === '') {
      lastindex = 0;
      var currentRecv = [];
    } else {
      lastindex = parseInt(
      ...
      var currentRecv = [...currProd['batch']];
    }
    ...
    const renderReceivingScan =
      currentRecv &&
      currentRecv.map((todo, i) => {
        return (
          <tr md="1" sm="3" xs="3" key={i}>
            <td>
              {todo.rcvd_ed === undefined ||
              todo.rcvd_ed === '0001-01-01T00:00:00Z' ? (
                <Label>{''}</Label>
              ) : (
                <Label className="text-left">
                  {new Date(todo.rcvd_ed).toDateString()}
                </Label>
              )}
            </td>
            <td>
              <Label className="text-left">{todo.rcvd_nobatch}</Label>
            </td>
            <td>
            ...
            <td>
              ...
              todo.rcvd_ed ? (
                ...
                <Button
                  ...
                  value={i}
                  disabled={!isSave}
                  onClick={this.saveBatchUpdate({ ...currProd }, lastindex)}
                >
                  <MdSave />
                </Button>
              )}
                ...
                <FormGroup>
                  <CardBody id="receivingUpper" style={{ display: 'none' }}>
                    ...
                    <Row md="1" sm="3" xs="3" style={{ marginBottom: '10px' }}>
                      ...
                      {/* {console.log('HEADER ISINYA: ', headerPO)} */}
                      {/* {console.log(
                        'DATE: ',
                        new Date(headerPO && headerPO.rcvh_tgldo),
                      )} */}
                      <Input
                        type="date"
                        className="text-left"
                        style={{ width: '20%' }}
                        value={
                          headerPO && headerPO.rcvh_tglfaktur === undefined
                            ? ''
                            : new Date(headerPO.rcvh_tglfaktur)
                                .toJSON()
                                .slice(0, 10)
                        }
                        onChange={event =>
                    ...
                    <Row md="1" sm="3" xs="3" style={{ marginBottom: '10px' }}>
                      ...
                      <Input
                        ...
                        style={{ width: '20%' }}
                        value={
                          headerPO && headerPO.rcvh_tgldo === undefined
                            ? ''
                            : new Date(headerPO.rcvh_tgldo)
                                .toJSON()
                                .slice(0, 10)
                        }
                        // value={
                        //   headerPO &&
                        //   headerPO.rcvh_tgldo === '0001-01-01T00:00:00Z'
                        //     ? new Date().toJSON().slice(0, 10)
                        //     : (this.isEmpty(headerPO)? new Date().toJSON().slice(0, 10) : new Date(headerPO.rcvh_tgldo).toJSON().slice(0,10))
                        // }
                        onChange={event =>
                          this.validateTgl(
                            event.target.value,
                      ...
                      ></Input>
                      {/* {console.log("HILIH: ",headerPO.tgl)} */}
                    </Row>
                    <Row
                      md="1"
                      sm="3"
                    ...
                    >
                      ...
                      <Input
                        className="text-left"
                        defaultValue={headerPO && headerPO.rcvh_niprecv}
                        style={{ width: '20%', textAlign: 'center' }}
                        onKeyDown={event => {
                          if (event.key === 'Enter') {
                            this.getEnterForKaryawan(event.target.value);
                          } else if (event.key === 'Backspace') {
                            if (this.state.namaKaryawan !== '')
                              this.setState({ namaKaryawan: '' });
                          }
                        }}
                      ></Input>
                      <Label
                      ...
                      <Input
                        ...
                        value={
                          ...
                          this.state.namaKaryawan === ''
                            ? '-'
                            : this.state.namaKaryawan === '' &&
                              this.state.loadingKaryawan === false
                            ? 'Sedang mencari...'
                            : this.state.namaKaryawan
                        }
                      />
              ...
              <Card style={{ marginBottom: '1%' }}>
                <CardHeader>
                  <CardBody>
                    <FormGroup>
                      <Row>
                        <Label style={{ fontWeight: 'bold', width: '7%' }}>
                          PROCOD
                        </Label>
                        <Input
                      ...
                      <Row id="qtyPOBonus" style={{ marginTop: '1%' }}>
                        <Col>
                          <Row>
                            <Label
                              style={{
                      ...
                      <Row id="satuanSellpack" style={{ display: 'none' }}>
                        <Label
                          style={{
                            color: 'red',
                            textAlign: 'center',
                        ...
                        >
                          <Badge color="danger">
                            {' '}
                            <MdInfo /> Mohon input qty Recv dalam satuan&nbsp;{' '}
                            {!this.isEmpty(currProd) && currProd.rcvd_sellpack
                              ? currProd.rcvd_sellpack
                              : null}
                          </Badge>
                        </Label>
                          ...
                          <tr id="buttonAdd" hidden>
                            ...
                            <td>
                              <Input
                                ...
                                value={lastBatch['rcvd_nobatch'].trim()}
                                id="rcvd_nobatch"
                                style={{ marginRight: '1%' }}
                                autoComplete="off"
                              ></Input>
              ...
              <CardBody>
                ...
                <Row>
                  <Col style={{ marginBottom: 0, paddingBottom: 0 }}>
                    ...
                    >
                      *Jika product kosong, tidak perlu discan dan tidak perlu
                      ditambahkan, serta product akan otomatis tidak direceive
                      apabila qty recv kosong.
                    </Label>
                  </Col>
                </Row>
                <hr></hr>
              ...
              <Card>
                <CardBody>
                  ...
                  >
                    ...
                    <ModalBody>
                      <Label>Apakah Anda yakin ingin menyimpan data ini?</Label>
                      <Label style={{ fontSize: '0.8em', color: 'red' }}>
                        *Perhatikan kembali NIP, tanggal Faktur dan tanggal
                        Surat jangan sampai kosong dan Salah!
                      </Label>
                    </ModalBody>
                    <ModalFooter>
                      <Button
                        color="primary"
                        onClick={this.confirmPO}
                        disabled={loading}
                      >
                        {!loading && 'Ya'}
                        {loading && <MdAutorenew />}
