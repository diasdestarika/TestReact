...
class View extends React.Component {
        ...
        try {
            this.setState({ currentNoReceive: this.props.location.state.noReceive, currentSelectedCategory: this.props.location.state.category },
                () => {
                    ...
                    this.props.setTitle('JUMLAH TN-IN ' + window.localStorage.getItem('gName'), 'red');
                });
        } catch (err) {
        ...
        return (
            this.state.productList.map(product => (
                <tr style={{ textAlign: 'center' }}>
                    ...
                    <td style={{ width: '10%' }}>{product.TranrcD_BatchNumber}</td>
    ...
    updateNewBatchInputValue(evt) {
        this.setState({
            ...
            if (this.state.newSelectedItemBatch === "") {
