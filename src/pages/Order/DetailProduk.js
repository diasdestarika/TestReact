...
class DetailProduk extends React.Component {
        ...
        fetch(urlA, option)
            .then(response => {
                ...
                if (data.responseMessage === "FALSE") {
                    ...
                    this.setState({ result2: data, maxPage: data.metadata.pages && data.metadata.pages !== "0" ? data.metadata.pages : 1, loading: false });
                }
            }).catch((err) => {
                this.showNotification("Koneksi ke server gagal!", 'error');
            }
    ...
    componentDidMount() {
        this.props.setTitle("Laporan Detail Product Kurang Stock di Gudang", 'black');
        this.getListbyPaging1(this.state.currentPage, this.state.todosPerPage);
