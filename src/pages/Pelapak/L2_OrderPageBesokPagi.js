...
class OrderPage extends React.Component {
  ...
  getListbyPaging(currPage, currLimit) {
    // const trace = perf.trace('getBesokPagi');
    // trace.start();
  ...
  componentDidMount() {
    this.props.setTitle(
      'Pesanan Yang Harus Selesai dan Cetak Resi Besok Pagi (L2)',
      '#FC8B03',
    );
    this.getListbyPaging(this.state.currentPage, this.state.todosPerPage);
  ...
  getAllEcommerce() {
    var gudangID = window.localStorage.getItem('gID');
    const urlA =
      myUrl.url_pilihEcommerce + 'type=rogd' + '&gudangID=' + gudangID;
    const option = {
      method: 'GET',
      json: true,
      headers: {
  ...
  render() {
    ...
    return (
      <Page
        // title="Pesanan Yang Harus Selesai Besok Pagi"
        // breadcrumbs={[{ name: 'Order', active: true }]}
        className="OrderPageBesokPagi"
      >
        <Row>
          <Col>
            <Card className="mb-3">
