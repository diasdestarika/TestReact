...
class OrderPage extends React.Component {
  ...
  getListbyPaging(currPage, currLimit) {
    // const trace = perf.trace('getBelumJadi');
    // trace.start();
  ...
  componentDidMount() {
    this.props.setTitle(
      'Order Pelapak Yang Belum Jadi SP Lebih Dari 1 Jam (L9)',
      'orange',
    );
    this.getListbyPaging(this.state.currentPage, this.state.todosPerPage);
  ...
  render() {
      ...
      currentTodos.map((todo, i) => {
        return (
          <tr key={i}>
            ...
            <td style={{ fontSize: '25px' }}>{todo.Order_Produk}</td>
    ...
    return (
      <Page
        // title="Pesanan Yang Harus Selesai Besok Sore"
        // breadcrumbs={[{ name: 'Order', active: true }]}
        className="OrderPageBesokSore"
      >
        <Row>
          <Col>
            <Card className="mb-3">
                ...
                <Table responsive striped>
                  <thead>
                    <tr style={{ width: '200%' }}>
                      ...
                      <th style={{ fontSize: '25px' }}>Produk</th>
